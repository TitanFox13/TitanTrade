...
**Fix**:
- `scheduler.py`: `_load_schedule_doc()` reads the full document; `start_scheduler()` passes `timezone=doc["timezone"]` to `BackgroundScheduler` (default `UTC` for back-compat; APScheduler 3.x resolves the IANA string via pytz). New module global `_schedule_timezone` so `_save_schedule()` (hit by the Flutter job-toggle path) preserves the tz instead of hard-coding `"UTC"`.
...
**Trade-offs**: jobs now fire at fixed ET wall-clock times, so their UTC instant shifts by an hour across DST boundaries (intended). Transition-day edge cases (spring-forward 02:00–03:00 gap, fall-back repeat) don't apply — no job runs near 02:00 ET.
**Validation**: 434 tests passing (was 431) — 3 new scheduler tests (configured tz applied, missing-tz defaults to UTC, toggle preserves tz). Existing scheduler tests unchanged (they use `"timezone": "UTC"`). Ruff clean.
