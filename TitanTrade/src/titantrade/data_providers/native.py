"""Native data provider — Alpaca + FRED + Finnhub (the default; ADR 040).

Implements the market-data interface (see ``data_providers/__init__.py``)
using services we already integrate or that are free:
  - **Alpaca** data API (existing keys, free IEX feed): bars, latest price,
    daily change %, news.
  - **FRED** (free key): VIX, treasury yields, economic-release calendar.
  - **Finnhub** (free key): earnings dates, analyst recommendations, sector.

Every function degrades to ``[]`` / ``None`` / ``{}`` on a missing key or
error, so a provider outage never crashes the pipeline (the macro/earnings
gates already treat missing data as "skip").
"""
...
def get_daily_change_pct(ticker: str, cfg: Config) -> float | None:
    """Today's % change vs the previous daily close (Alpaca snapshot)."""
...
def get_news(ticker: str, cfg: Config, limit: int = 50) -> list[dict[str, Any]]:
    """Recent news from Alpaca's News API. Shape: ``[{title, snippet,
    published_at, source}]`` (de-dup applied by the caller)."""
...
def _fred_latest(series_id: str, cfg: Config) -> float | None:
    ...
    params = {
        "series_id": series_id, "api_key": cfg.fred.key, "file_type": "json",
...
def get_treasury_yields(cfg: Config) -> dict[str, float | None]:
    """10Y and 2Y Treasury yields (FRED DGS10 / DGS2)."""
    return {
...
# High-impact US economic releases, by FRED release_id. Per-release queries
# return upcoming scheduled dates reliably; the blanket /releases/dates endpoint
# does not surface near-term dates (its future entries span all ~300 releases).
_FRED_RELEASE_IDS = {
...
def get_economic_calendar(cfg: Config, days_ahead: int = 7) -> list[dict[str, Any]]:
    ...
    if cfg.fred.key:
        url = f"{cfg.fred.base_url}/release/dates"
        for rid, label in _FRED_RELEASE_IDS.items():
            params = {
                "release_id": str(rid), "api_key": cfg.fred.key,
            ...
            try:
                ...
                log.warning(f"FRED release {rid} ({label}) fetch failed: {exc}")
...
def get_earnings_dates(tickers: list[str], cfg: Config) -> dict[str, str | None]:
    ...
    ``{ticker: "YYYY-MM-DD" | None}``. All None when no Finnhub key.
    """
...
def get_analyst_ratings(ticker: str, cfg: Config) -> dict[str, Any]:
    """Analyst recommendation trend from Finnhub. ``{"recent_grades": [...]}``
    with the latest buy/hold/sell distribution. Empty when no key. (Price-target
    consensus has no free source post-FMP and is omitted.)"""
    result: dict[str, Any] = {}
    if not cfg.finnhub.key:
        return result
